...
export default class Title extends Phaser.Scene {
  ...
  create() {
    this.editorCreate();
    // Prevents player physics on title screen
    this.player.body.enable = false;
    const glowfx = this.player.postFX.addGlow(0x00ffff, 1, 0, false, 0.1, 5); //Glow effect on player
  ...
  //Funcion that is called when the player Clicks on the title screen
  startGame() {
    this.player.stop(); //stop player current animation
    this.player.setFrame("player-duck.png");
    this.time.delayedCall(1000, () => {
      this.player.play("playerIdle");
      this.glowTween.destroy(); //Removes player tween glow
      this.player.postFX.clear(); //Remove player glow
